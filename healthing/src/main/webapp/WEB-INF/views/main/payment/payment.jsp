...
<body>
	...
	<div class="paymentWarp">
		...
		<form action="/payment" method="post" name='frm_Input'>
					...
					<div class="paymentCoach">
						...
						<div class="paymemtCoachImg">
							...
							<div class="paymentSub">
								<p><b>${item.icoachname}</b> 코치가</p>
								<p><b>이주철</b> 코치가</p>
								<p>오늘부터 <b>${list.uname}</b>님의</p>
								<p>전담 코치가 됩니다.</p>
							</div>
						</div>
						<div class="paymemtProgram">
							...
							<div class="paymentProSubMain">
								<div>
									...
									<span class="paymentProSub"><b>이주철</b> 님</span>
									<span class="paymentProSub">${item.icoachname}님</span>
								</div>
								<div>
									<span class="paymentProTitle">프로그램</span>
									<span class="paymentProSub"><strong>철코치와 함께 하는 하드 해피타임</strong></span>
									<span class="paymentProSub"><strong>${item.ititle}</strong></span>
								</div>
								<div>
									<span class="paymentProTitle">온라인</span>
									<span class="paymentProSub"><b>3 주 / 주 5 회 / 회당 10 분</b></span>
									<span class="paymentProSub"><b>${item.iweek}주 / 주 ${item.iweekcount}회 / 회당 ${item.iweektime}분</b></span>
								</div>
							</div>
						</div> --%>
						<div class="paymentAddress">
							...
							<div class="paymentAddInput">
								<p>받는사람</p>
								<input type="text" value="${list.uname}">
							</div>
							<div class="paymentAddInput">
								<p>받는 사람 휴대폰전화번호</p>
								<input type="text" value="${list.uphone}">
							</div>
							<div class="paymentAddInput">
								<p>주소</p>
								<div class="paymentAddInputSub">
									<input type="text" value="${list.uaddr1}" placeholder="우편번호 찾기로 주소를 입력하세요">
									<div class="paymentAddAppend">
										<button>우편번호 찾기</button>
									</div>
								</div>
								<input type="text" value="${list.uaddr2}" placeholder="상세 주소를 입력하세요">
							</div>
						</div>
					</div>
				</div>
				<div class="paymentRight">
					<div class="paymentInfo">
						...
						<div>
							...
							<p class="paymentInfoSub">${list.uname}</p>
							<p class="paymentInfoTit">휴대폰전화번호</p>
							<p class="paymentInfoSub">${list.uphone}</p>
						</div>
						<%-- <div class="paymentAmount">
							<p>결제금액</p>
							<div>
								<span>300,000</span>
								<span>${item.iprice}-(${item.iprice}*(${item.isale}*0.01))</span>
								원
							</div>
						</div> --%>
