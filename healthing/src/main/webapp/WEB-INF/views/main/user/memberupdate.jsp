...
<div id="memberjoinMain">
	<div class="memberjoinDetail">
		...
		<form action="/memberUpdate" method="post" name='frm_Input' onsubmit="return uphoneFunc();">
			<table class="memberjoinTable">
				...
				<tr class="memberjoinName">
					<th>이름<span>*</span></th>
					<td>
						<input type="text" name="uname" value="${list.uname}" required>
					</td>
				</tr>
				<tr class="memberjoinPhone">
					<th>연락처<span>*</span></th>
					<td>
						<input type="text" name="uphone" id="uphone" maxlength="11" value="${list.uphone}" required>
				...
				<tr class="memberjoinEmail">
					<th>이메일<span>*</span></th>
					<td>	
						<c:set var="text" value="${list.uemail}"/>
						<input type="text" name="uemail1" id="emailId" class="form_w200" value="${fn:split(text, '@')[0]}" title="이메일 아이디" maxlength="20" required /> @ 
						<input type="text" name="uemail2" id="emailDomain" class="form_w200" value="${fn:split(text, '@')[1]}" title="이메일 도메인" maxlength="20" required />
				...
				<tr class="memberjoinBirth">
					<th>생년월일</th>
					<td>
						<c:set var="text" value="${list.ubirth}"/>
				...
				<tr class="memberjoinGender">
					...
					<td>
						<c:choose>
							...
							<c:otherwise>
								<label><input type="radio" name="ugender" id="man" value="man" required>남자</label>
								<label><input type="radio" name="ugender" id="woman" value="woman" checked required>여자</label>
							</c:otherwise>
						</c:choose>
					</td>
				</tr>
				<tr class="memberjoinheight">
					<th>키<span>*</span></th>
					<td>
						<label><input type="text" name="ucm" maxlength="5" value="${list.ucm}" required> cm</label>
					</td>
				</tr>					
				<tr class="memberjoinweight">
					<th>몸무게<span>*</span></th>
					<td colspan="2">
						<label><input type="text" name="ukg" maxlength="5" value="${list.ukg}" required> kg</label>
					</td>
				</tr>
				<tr class="memberjoinZipcode">
					<th rowspan="4">주소</th>
					<td>
						<input type="text" name="uzipcode" id="zipcode" class="memberjoinZipnum" value="${list.uzipcode}">
						<input type="button" name='btnFindZip' onclick="kakaopost()" value="우편번호">
					</td>
				</tr>
				<tr class="memberjoinAddressone">
					<td>
						<input type="text" id="address1" name="uaddr1" value="${list.uaddr1}" >								
					</td>
				</tr>
				<tr class="memberjoinAddresstwo">
					<td>
						<input type="text" name="uaddr2" value="${list.uaddr2}">
					</td>
				</tr>
			...
			<div class="memberjoinBtn">
				<input type="submit" id="overlapBtn" value="저장">
			</div>
		</form>
	</div>
</div>
...
$(document).ready(function(){
	var getData = "${list.ubirth}";
	var setData = getData.split("-");
    var now = new Date();
    ...
    for(var i=1; i <= 12; i++) {
        ...
        if(i==setData[1]){
        	$('#month').append('<option value="' + mm + '" selected>' + mm + '</option>');
        }else{
    ...
    for(var i=1; i <= 31; i++) {
        ...
        if(i==setData[2]){
        	$('#day').append('<option value="' + dd + '" selected>' + dd + '</option>');
        }else{
