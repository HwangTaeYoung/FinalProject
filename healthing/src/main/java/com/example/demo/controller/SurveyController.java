...
public class SurveyController {
	...
	public String surveyallviewController(Model model)throws Exception {
		
		model.addAttribute("list",msurveyService.surveyHeathService());
		//model에 데이터를 담기위해 addAttribute 사용 ( "String name", Object value )
	
		return "/main/survey/surveyallview";
	}
	//surveymuscle
	@RequestMapping(value="/surveymuscle")	
	public String surveymuscleController(Model model)throws Exception {
	
		model.addAttribute("list",msurveyService.surveyMuscleService());
		//model에 데이터를 담기위해 addAttribute 사용 ( "String name", Object value )
	
		return "/main/survey/surveymuscle";
	}
	//surveybodytype
	@RequestMapping(value="/surveybodytype")	
	public String surveybodytypeController(Model model)throws Exception {
			
		model.addAttribute("list",msurveyService.surveyBodytypeService());
		//model에 데이터를 담기위해 addAttribute 사용 ( "String name", Object value )
		
		return "/main/survey/surveybodytype";
	}
	...
	public String surveyresultController(HttpServletRequest request,Model model)throws Exception {
		...
		int sessionuno = msurveyService.surveyResultService(sessionId); //String 값을 int형 변수로 바꿔줌
		
		msurveyService.surveySessionService(sessionuno); //바꾼 값을 서비스단으로 전달
		
		SurveyVO surveyVO = msurveyService.surveySessionService(sessionuno); // xml-매퍼-서비스단 통해 다시 전달받은 값을 SurveyVO타입의 변수명 지정
		
		model.addAttribute("result", surveyVO); // 변수명 지정한 값을 model.addAttribute를 통해 뷰단으로 전달 키값은 result
